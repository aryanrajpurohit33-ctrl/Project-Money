...
function renderCustomerLoginForm(container) {
  container.innerHTML = `
    <div class="glass max-w-md mx-auto rounded-3xl p-6 sm:p-8 space-y-6 font-sans animate-fadeIn">
      ...
      <form onsubmit="handleCustomerLoginSubmit(event)" class="space-y-4 font-mono text-xs">
        ...
        <button type="submit" class="w-full py-3.5 rounded-xl bg-emerald-500 text-gray-950 font-black uppercase tracking-wider shadow-lg font-sans text-xs cursor-pointer active:scale-95 transition-all">Sign In</button>
      </form>

      <div class="pt-4 border-t border-white/5 text-center text-xs font-sans text-slate-400">
        New to Nexus Digital? 
        <button onclick="renderCustomerRegisterForm(document.getElementById('storeContent') || document.getElementById('mainViewport'))" class="text-emerald-400 font-bold hover:underline ml-1 cursor-pointer">Create an account</button>
      </div>
    </div>
  `;
}
  ...
  container.innerHTML = `
    <div class="glass max-w-md mx-auto rounded-3xl p-6 sm:p-8 space-y-6 font-sans animate-fadeIn">
      ...
      <form onsubmit="handleCustomerRegisterSubmit(event)" class="space-y-4 font-mono text-xs">
        ...
        <button type="submit" class="w-full py-3.5 rounded-xl bg-emerald-500 text-gray-950 font-black uppercase tracking-wider shadow-lg font-sans text-xs cursor-pointer active:scale-95 transition-all">Create Account</button>
      </form>

      <div class="pt-4 border-t border-white/5 text-center text-xs font-sans text-slate-400">
        Already a customer? 
        <button onclick="renderCustomerLoginForm(document.getElementById('storeContent') || document.getElementById('mainViewport'))" class="text-emerald-400 font-bold hover:underline ml-1 cursor-pointer">Login here</button>
      </div>
    </div>
  `;
}
