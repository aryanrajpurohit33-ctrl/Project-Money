...
async function renderStoreProductDetails(container, productId) {
  ...
  const localMatch = Array.isArray(cachedList) ? cachedList.find(x => x._id === productId || x.slug === productId) : null;

  if (localMatch) {
    paintProductDetailPage(container, localMatch);
  } else {
  ...
  try {
    ...
    paintProductDetailPage(container, p);
  } catch (err) {
    if (!localMatch) {
      container.innerHTML = `
...
function paintProductDetailPage(container, p) {
          ...
          <div class="relative" onclick="event.stopPropagation()">
            ...
            <div id="durationDropdownMenu" class="hidden absolute top-full left-0 right-0 mt-2 space-y-1 p-2 rounded-2xl bg-surface-900/95 backdrop-blur-xl border border-white/10 shadow-2xl z-30 transform transition-all duration-300 opacity-0 -translate-y-2">
              <div onclick="selectDurationCustom('1_MONTH', 1, 50, '1 Month (Standard Plan)', 'STANDARD PLAN')" 
                class="p-3 rounded-xl hover:bg-white/5 flex items-center justify-between cursor-pointer">
              ...
              <div onclick="selectDurationCustom('1_YEAR', 7.2, 40, '1 Year / 12 Months (Best Value — Save 40% OFF)', 'BEST VALUE 40% OFF')" 
                class="p-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 flex items-center justify-between cursor-pointer">
                <div class="flex items-center gap-2">
        ...
        <div class="pt-2 flex items-center justify-between gap-2">
          ...
          <div class="flex flex-col items-end gap-1">
            ...
            <div class="flex items-center gap-2 bg-surface-900 p-1.5 rounded-2xl border border-white/5">
              ...
              <span class="text-xs font-mono font-bold text-emerald-400 min-w-[65px] text-center" id="deviceDisplayCount">1 Device</span>
        ...
        <div class="space-y-2.5 text-[11px] text-slate-300 font-sans leading-relaxed">
          ...
          ` : `
            <div class="flex items-start gap-2">
              ...
              <span><strong class="text-white">Do NOT Share Password:</strong> Keep login credentials strictly confidential.</span>
            </div>
            <div class="flex items-start gap-2">
              <span class="text-rose-400 font-bold text-xs leading-none mt-0.5">•</span>
              <span><strong class="text-white">1 Device per Purchase:</strong> You can only log in on 1 device at a time unless additional devices were purchased.</span>
            </div>
          `}
        </div>
      </div>
