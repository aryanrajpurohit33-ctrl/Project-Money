...
async function renderAdminPayments(container) {
  ...
  try {
    ...
    paintAdminPaymentSettingsHTML(container, cfg || {});
  } catch (err) {
    container.innerHTML = `
...
function paintAdminPaymentSettingsHTML(container, cfg) {
  window.currentUploadedQRCode = cfg.upi_qr_image || cfg.qr_code || '';
  ...
  container.innerHTML = `
    <div class="space-y-6 font-sans text-xs pb-24 max-w-2xl mx-auto animate-fadeIn px-1">
      ...
      <form onsubmit="handleSavePaymentGatewaySettings(event)" class="space-y-5">
        ...
        <!-- UPI Payment Settings Card (Crypto completely removed) -->
...
function renderQRPreviewHTML() {
  if (!window.currentUploadedQRCode) {
    return `
      ...
      <p class="text-slate-400 font-mono text-[10px] leading-tight">Dynamic QR code will be generated on checkout using your UPI ID.</p>
    `;
  }
  ...
  return `
    ...
    <div class="space-y-1">
      ...
      <p class="text-slate-400 font-mono text-[10px] leading-tight">This QR image will be shown on customer checkout.</p>
    </div>
  `;
}
...
async function handleSavePaymentGatewaySettings(e) {
  ...
  try {
    ...
    renderAdminPayments(document.getElementById('adminMainContent'));
  } catch (err) {
