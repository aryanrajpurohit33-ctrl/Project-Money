...
async function renderAdminOverview(container) {
  const cached = window.apiCache?.get('/api/admin/dashboard')?.data;
  if (cached) {
    paintDashboardHTML(container, cached);
  } else {
  ...
  try {
    ...
    paintDashboardHTML(container, data);
  } catch (err) {
    if (!cached) {
      container.innerHTML = `<div class="admin-card p-8 text-center text-rose-400 text-xs font-mono rounded-3xl">Error: ${err.message}</div>`;
    }
  }
}

function paintDashboardHTML(container, data) {
  ...
  container.innerHTML = `
    <div class="space-y-6 font-sans text-xs pb-24 w-full max-w-full animate-fadeIn">
      ...
      ${pendingOrders > 0 ? `
        <div class="p-4 rounded-3xl bg-amber-500/10 border border-amber-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xl">
          <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-2xl bg-amber-500/20 text-amber-300 font-mono font-bold flex items-center justify-center text-sm shrink-0">⏳</div>
            <div>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 font-mono">
        <div class="admin-card p-4 sm:p-5 rounded-3xl space-y-2 border border-admin-border relative overflow-hidden group hover:border-emerald-500/30 transition-all shadow-xl">
          ...
          <span class="text-[10px] ${totalRev > 0 ? 'text-emerald-400 font-bold' : 'text-slate-500'} block">${totalRev > 0 ? '↑ Verified Settlement' : '₹' + pipelineRev + ' In Pipeline'}</span>
        </div>
        ...
        <div class="admin-card p-4 sm:p-5 rounded-3xl space-y-2 border border-admin-border relative overflow-hidden group hover:border-amber-500/30 transition-all shadow-xl">
          ...
          <span class="text-[10px] ${pendingOrders > 0 ? 'text-amber-400 font-bold' : 'text-slate-500'} block">${pendingOrders > 0 ? 'Needs Verification' : 'Queue Clear'}</span>
        </div>
      ...
      <div class="admin-card p-5 sm:p-6 rounded-3xl border border-admin-border space-y-5 shadow-2xl">
        <div class="flex items-center justify-between">
          ...
          <span class="px-3 py-1 rounded-xl bg-surface-950 border border-white/5 font-mono text-[10px] text-emerald-400 font-bold">${totalRev > 0 ? 'Peak: ₹' + maxRevVal : 'Waiting for verified order'}</span>
        </div>
          ...
          ${timeline.map(t => {
            ...
            return `
              <div class="flex flex-col items-center gap-2 group h-full justify-end">
                <span class="text-[9px] ${hasRev ? 'text-emerald-400 font-bold' : 'text-slate-600'} opacity-0 group-hover:opacity-100 transition-opacity">₹${t.revenue}</span>
                <div class="w-full bg-surface-950 rounded-2xl p-1 h-32 flex items-end border border-white/5">
                  <div class="w-full ${hasRev ? 'bg-gradient-to-t from-emerald-600 to-teal-400 shadow-lg shadow-emerald-500/20' : 'bg-surface-900'} rounded-xl transition-all duration-700" style="height: ${heightPct}%;"></div>
                </div>
      ...
      <div class="admin-card p-5 sm:p-6 rounded-3xl border border-admin-border space-y-4 shadow-2xl">
        ...
        <div class="space-y-3">
          ${topProducts.length === 0 ? `
            <div class="py-10 text-center space-y-2 font-mono text-xs text-slate-400">
              <span class="text-2xl block">📊</span>
              <p>No verified sales recorded yet.</p>
            </div>
