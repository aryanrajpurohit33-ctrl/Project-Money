...
function preventTouchScroll(e) {
  if (!e.target.closest('#instructionModalContent')) {
    e.preventDefault();
  }
}
...
async function renderStoreCheckout(container) {
  ...
  // Clean up any existing global instruction modals
  const oldModal = document.getElementById('globalInstructionModalRoot');
  ...
  // Attach modal overlay directly to document.body for true center viewport pinning
  const modalContainer = document.createElement('div');
  modalContainer.id = 'globalInstructionModalRoot';
  modalContainer.innerHTML = `
    <div id="instructionModalOverlay" onclick="closeInstructionsModal()" class="fixed inset-0 z-[99999] bg-black/85 backdrop-blur-md flex items-center justify-center p-4 opacity-0 pointer-events-none transition-opacity duration-200">
      
      <div id="instructionModalContent" onclick="event.stopPropagation()" class="w-full max-w-sm bg-surface-900 border border-white/10 rounded-3xl p-5 space-y-3.5 shadow-2xl transform scale-90 transition-all duration-200 font-sans mx-auto">
        
        <!-- Header -->
        <div class="flex items-center justify-between border-b border-white/5 pb-2.5">
          <div class="flex items-center gap-2">
            <span class="text-amber-400 text-base">⚠️</span>
            <h3 class="text-white font-black text-xs uppercase tracking-wide font-mono">Usage Policy & Rules</h3>
          </div>
        ...
        <div class="space-y-1.5 pt-1 font-mono">
          ...
          <button type="button" onclick="closeInstructionsModal()" class="w-full py-1 text-center text-slate-500 hover:text-slate-300 text-[10px] uppercase font-bold cursor-pointer">
            Cancel
          </button>
        </div>
...
function openInstructionsModal() {
  ...
  // Instant scroll lock on viewport and touch
...
function closeInstructionsModal() {
  ...
  // Remove scroll and touch freeze
